...
## close(blogsURL, newsURL, twitterURL)
## rm(blogsURL, newsURL, twitterURL)
```

## 4 - SOME DATA STATISTICS

Let's evaluate the Data loaded from Blogs, News and Twitter files.
...
* Twitter is the big file with more lines, and fewer words per line (as expected 140 lines limited).
* Blogs are the text file with sentences and has the longest line with 40,833 characters.
* News are the text file with sentences.
* Twitter are the text file with sentences until 140 characters as would be expected and with 2,360,148 number of lines.
...
```{r, eval=FALSE}
set.seed(65364)
sTwitter <- sample(twitter, size = 30000, replace = TRUE)
...
The new file is composed of 1/3 of lines from each one of the records selected (from blogs, news, and Twitter)
...
## Stripping unnecesary whitespace from document
...
rm(blogs, news, twitter); rm(sNews, sTwitter); rm(sBlogs)
rm(textCorpus, textCon, finalCorpusMem, removeURL, blogsURL, newsURL, twitterURL, sampleTotal)
```
